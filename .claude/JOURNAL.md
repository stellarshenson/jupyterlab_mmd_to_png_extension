...
13. **Task - README Documentation Rewrite**: Rewrote README.md following modus primaris principles<br>
    **Result**: Restructured README with clear opening statement explaining core functionality. Organized features into "Core capabilities" (copy/save, DPI configuration, transparent backgrounds, deterministic filenames) and "Technical implementation" (client-side rendering, DPI calibration accuracy). Updated all documentation to reflect current menu labels ("Copy as PNG", "Save as PNG") and viewer-only context menu behavior. Streamlined Installation, Usage, Troubleshooting, and Contributing sections with flowing narrative and concise instructions. Removed outdated references to editor mode. Added specific technical details including DPI ranges (72-1200, default 300), filename pattern (mermaid-<document-name>-<content-hash>.png), and calibration accuracy (±0.07% matching Adobe converter)
